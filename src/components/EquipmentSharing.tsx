    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      {loading && (
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 text-[#8B7355] dark:text-[#C9A87C] animate-spin" />
          <span className="ml-3 text-gray-600 dark:text-[#B8B8B8]">Loading shared equipment...</span>
        </div>
      )}
                ...
                >
                  <div className="p-6">
                    ...
                    <div className="pt-4 border-t border-[#E8DCC9] dark:border-[#4A4A4A]">
                      ...
                      {item.ownerId === parseInt(user.id) && (
                        <button 
                          className="w-full px-4 py-2 bg-[#F5E6D3] dark:bg-[#3D3D3D] text-[#8B7355] dark:text-[#C9A87C] rounded-lg hover:bg-[#E8DCC9] dark:hover:bg-[#4A4A4A] transition-colors"
                          disabled
                        >
                          Your Item
                        </button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="bg-white dark:bg-[#2D2D2D] rounded-lg shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto border border-[#E8DCC9] dark:border-[#4A4A4A] animate-scaleIn">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-[#8B7355] dark:text-[#C9A87C]">Share Your Equipment</h2>
              <button
                onClick={() => {
                  setShowAddForm(false);
                  setFormError(null);
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              <div>
                <label className="block text-gray-700 dark:text-[#B8B8B8] mb-2">Equipment Name</label>
                <input
                  type="text"
                  required
                  value={toolName}
                  ...
                  className="w-full px-4 py-2 border border-[#E8DCC9] dark:border-[#4A4A4A] bg-white dark:bg-[#3D3D3D] text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#8B7355] dark:focus:ring-[#C9A87C]"
                />
              </div>

              <div>
                <label className="block text-gray-700 dark:text-[#B8B8B8] mb-2">Description</label>
                <textarea
                  required
                  value={description}
              ...
              <div>
                <label className="block text-gray-700 dark:text-[#B8B8B8] mb-2">Image URL (Optional)</label>
                <input
                  type="url"
                  value={imageUrl}
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  disabled={submitting}
                  className="flex-1 px-4 py-2 bg-gray-200 dark:bg-[#3D3D3D] text-gray-700 dark:text-[#B8B8B8] rounded-lg hover:bg-gray-300 dark:hover:bg-[#4A4A4A] transition-colors disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 px-4 py-2 bg-gradient-to-r from-[#8B7355] to-[#A0826D] dark:from-[#C9A87C] dark:to-[#A0826D] text-white rounded-lg hover:shadow-lg transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {submitting ? (
                    <>
