...
export function LabRooms({ user, onNavigateToReservation }: LabRoomsProps) {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      {/* Header */}
      <div>
        <h1 className="text-[#5B9BD5] dark:text-[#8BB4D9]">Lab Rooms</h1>
      ...
      <div className="bg-white dark:bg-[#252932] rounded-lg shadow p-4 border-2 border-[#CBD5E0] dark:border-[#3A4150]">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 border-2 border-[#CBD5E0] dark:border-[#3A4150] bg-white dark:bg-[#2F3541] text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5B9BD5] dark:focus:ring-[#8BB4D9]"
          />
        </div>
      </div>
      ...
      {loading && (
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-3 text-gray-600 dark:text-[#B0B0B0]">Loading labs...</span>
        </div>
      )}
      ...
      {!loading && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredLabs.length === 0 ? (
            <div className="col-span-full text-center py-12 text-gray-600 dark:text-[#B0B0B0]">
            ...
            filteredLabs.map((lab, index) => (
              <div
                key={lab.id}
                className="bg-white dark:bg-[#252932] rounded-lg shadow hover:shadow-xl transition-all duration-300 transform hover:scale-105 cursor-pointer animate-scaleIn border-2 border-[#CBD5E0] dark:border-[#3A4150]"
                style={{ animationDelay: `${index * 0.05}s` }}
                onClick={() => setSelectedLab(lab)}
              >
                <div className="p-6">
                  <h3 className="mb-3 text-[#5B9BD5] dark:text-[#8BB4D9]">{lab.name}</h3>
                  ...
                  <button className="w-full mt-4 flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-[#FF9F66] to-[#5B9BD5] text-white rounded-lg hover:shadow-lg transition-all">
                    View Equipment
      ...
      {selectedLab && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="bg-white dark:bg-[#252932] rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-scaleIn border-2 border-[#FF9F66]">
            <div className="sticky top-0 bg-gradient-to-r from-[#5B9BD5] to-[#8BB4D9] text-white p-6 rounded-t-lg">
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-white mb-2">{selectedLab.name}</h2>
                  <p className="text-[#CBD5E0] text-sm">{selectedLab.building} - Floor {selectedLab.floor}</p>
                </div>
                <button
            ...
            <div className="p-6">
              <h3 className="mb-4 text-[#5B9BD5] dark:text-[#8BB4D9]">Available Equipment</h3>
              
              {equipmentLoading === selectedLab.id ? (
                <div className="flex items-center justify-center py-8">
                  <Loader2 className="w-6 h-6 text-[#5B9BD5] dark:text-[#8BB4D9] animate-spin" />
                  <span className="ml-3 text-gray-600 dark:text-[#B0B0B0]">Loading equipment...</span>
                </div>
              ) : selectedLab.equipment.length === 0 ? (
                <div className="text-center py-8 text-gray-600 dark:text-[#B0B0B0]">
                ...
                <div className="space-y-3">
                  {selectedLab.equipment.map((equipment) => (
                    <div
                      key={equipment.id}
                      className="flex items-center justify-between p-4 bg-[#F5F7FA] dark:bg-[#2F3541] rounded-lg hover:bg-[#CBD5E0] dark:hover:bg-[#3A4150] transition-colors border-2 border-[#CBD5E0] dark:border-[#3A4150]"
                    >
