...
interface LoginProps {
  ...
  onShowRegister: (type?: string) => void;
}
...
export function Login({ onLogin, onShowRegister }: LoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userId = response.studentId || response.instructorId || response.technicianId || response.doctorId || 0;
      const user: User = {
        id: userId.toString(),
        name: response.name,
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] dark:from-[#1A1D23] dark:via-[#252932] dark:to-[#1A1D23] flex items-center justify-center p-4 animate-fadeIn">
      <div className="w-full max-w-md">
        {/* Logo and Title */}
        <div className="text-center mb-8 animate-scaleIn">
          <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-[#1E3A5F] to-[#2C5282] dark:from-[#8BB4D9] dark:to-[#5B9BD5] rounded-full mb-4 shadow-lg">
            <Beaker className="w-10 h-10 text-white" />
          </div>
          <h1 className="text-[#1E3A5F] dark:text-[#8BB4D9] mb-2">Badya University</h1>
        ...
        <div className="bg-[#F8F9FA] dark:bg-[#252932] rounded-lg shadow-2xl p-8 animate-slideInLeft border-2 border-[#E2E8F0] dark:border-[#3A4150]">
          <h2 className="mb-6 text-center text-[#1E3A5F] dark:text-[#8BB4D9]">Sign In</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                required
                className="w-full px-4 py-3 border-2 border-[#E2E8F0] dark:border-[#3A4150] bg-[#FFFFFF] dark:bg-[#2F3541] text-[#1A202C] dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1E3A5F] dark:focus:ring-[#8BB4D9] focus:border-transparent transition-all"
              />
            </div>
          ...
          <div className="mt-6 space-y-2">
            <button
              ...
              className="flex items-center justify-center gap-2 w-full text-sm text-[#1E3A5F] dark:text-[#8BB4D9] hover:underline"
            >
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                onClick={() => onShowRegister('instructor')}
                className="text-xs px-3 py-2 bg-[#1E3A5F]/10 dark:bg-[#8BB4D9]/10 text-[#1E3A5F] dark:text-[#8BB4D9] rounded-lg hover:bg-[#1E3A5F]/20 dark:hover:bg-[#8BB4D9]/20 transition-all"
              >
                Instructor
              </button>
              <button
                onClick={() => onShowRegister('technician')}
                className="text-xs px-3 py-2 bg-[#1E3A5F]/10 dark:bg-[#8BB4D9]/10 text-[#1E3A5F] dark:text-[#8BB4D9] rounded-lg hover:bg-[#1E3A5F]/20 dark:hover:bg-[#8BB4D9]/20 transition-all"
              >
                Technician
              </button>
