...
export function RegisterInstructor({ onRegister, onBackToLogin }: RegisterInstructorProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const user: User = {
        id: (response.instructorId || 0).toString(),
        name: response.name,
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#FAF9F5] via-[#EADFB4] to-[#9BB0C1] dark:from-[#1A1D23] dark:via-[#252932] dark:to-[#1A1D23] flex items-center justify-center p-4 animate-fadeIn">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white dark:bg-[#252932] rounded-lg shadow-2xl p-8 animate-slideInLeft border-2 border-[#EADFB4] dark:border-[#3A4150]">
          <h2 className="mb-6 text-center text-[#51829B] dark:text-[#9BB0C1]">Create Instructor Account</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Instructor ID */}
            <div>
              <label htmlFor="instructorId" className="block text-gray-700 dark:text-[#B0B0B0] mb-2">
                Instructor ID *
              </label>
              <input
                id="instructorId"
              ...
              />
              <p className="text-xs text-gray-500 dark:text-[#6B6B6B] mt-1">Must match your registered Instructor ID</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border-2 border-[#EADFB4] dark:border-[#3A4150] bg-white dark:bg-[#2F3541] text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#51829B] dark:focus:ring-[#9BB0C1] focus:border-transparent transition-all"
              />
              <p className="text-xs text-gray-500 dark:text-[#6B6B6B] mt-1">Must match the name registered with your Instructor ID</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border-2 border-[#EADFB4] dark:border-[#3A4150] bg-white dark:bg-[#2F3541] text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#51829B] dark:focus:ring-[#9BB0C1] focus:border-transparent transition-all"
              />
              <p className="text-xs text-gray-500 dark:text-[#6B6B6B] mt-1">Must match the email registered with your Instructor ID</p>
            </div>
