...
export function Register({ onRegister, onBackToLogin }: RegisterProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const user: User = {
        id: response.studentId.toString(),
        name: response.name,
    ...
    <div className="min-h-screen bg-[#FFFFFF] dark:from-[#1A1D23] dark:via-[#252932] dark:to-[#1A1D23] flex items-center justify-center p-4 animate-fadeIn">
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#F8F9FA] dark:bg-[#252932] rounded-lg shadow-2xl p-8 animate-slideInLeft border-2 border-[#E2E8F0] dark:border-[#3A4150]">
          <h2 className="mb-6 text-center text-[#1E3A5F] dark:text-[#2C5282]">Create Account</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Student ID */}
            <div>
              <label htmlFor="studentId" className="block text-[#1A202C] dark:text-[#B0B0B0] mb-2">
                Student ID *
              </label>
              <input
                id="studentId"
